...
EMAILS=$(cd ~/agent-workspace/skills/google-workspace && node scripts/gmail.js list user@example.com "after:${DATE//-/\\/} before:${DATE_NEXT//-/\\/} from:me" 2>/dev/null | jq 'length' || echo "?")
echo "  • Emails sent: $EMAILS"

# Slack messages
SLACK_TOKEN=$(cat ~/agent-workspace/secrets/slack-super-ada.json 2>/dev/null | jq -r '.user_token')
...
XCOM=$(ssh -o ConnectTimeout=10 henrymascot@<REDACTED_IP> "
...
FIREFLIES_KEY=$(cat ~/agent-workspace/secrets/fireflies.key 2>/dev/null)
...
# Git commits
GIT_COMMITS=$(cd ~/agent-workspace && git log --oneline --since="$DATE 00:00" --until="$DATE_NEXT 00:00" 2>/dev/null | wc -l | tr -d ' ')
echo "  • Git commits (clawd): $GIT_COMMITS"

# Google Docs edited (using Google Workspace skill)
DOCS=$(cd ~/agent-workspace/skills/google-workspace && node scripts/drive.js list user@example.com "modifiedTime>='${DATE}T00:00:00'" 2>/dev/null | jq 'length' || echo "?")
echo "  • Docs modified: $DOCS"

# Ada messages (from session)
ADA_MSGS=$(grep -c "Him (@<your-github-user>)" ~/.openclaw/agents/main/sessions/*.jsonl 2>/dev/null | tail -1 | cut -d: -f2 || echo "?")
...
AW_DATA=$(ssh -o ConnectTimeout=10 henrymascot@<REDACTED_IP> "
curl -s 'http://localhost:5600/api/0/buckets/aw-watcher-window_<your-mac>/events?limit=500' 2>/dev/null | python3 -c '
import json, sys
...
SCREEN_TIME_DATA=$(ssh -o ConnectTimeout=10 henrymascot@<REDACTED_IP> "python3 ~/get_screentime.py $DATE" 2>/dev/null)
