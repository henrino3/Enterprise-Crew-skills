...
~/agent-workspace/skills/daily-review/scripts/daily-review.sh

# Run for specific date
~/agent-workspace/skills/daily-review/scripts/daily-review.sh 2026-01-15
```
...
```bash
cp -r daily-review ~/agent-workspace/skills/
```
...
```bash
# Bird (X.com)
cat > ~/agent-workspace/secrets/bird.env << 'EOF'
AUTH_TOKEN=your_auth_token
CT0=your_ct0
EOF

# Fireflies
echo "your_api_key" > ~/agent-workspace/secrets/fireflies.key

# Slack
echo '{"user_token": "<REDACTED_TOKEN>"}' > ~/agent-workspace/secrets/slack-super-ada.json
```
