...
import { replaceEntityPaths } from "./rewrite-paths.js";
...
const discordMessageActions = {
  ...
  handleAction: async (ctx) => {
    const messageActions = getDiscordRuntime().channel.discord.messageActions;
    if (!messageActions?.handleAction) {
      throw new Error("Discord message actions not available");
    }
    return messageActions.handleAction(ctx);
  },
};

function buildDiscordSendArgs({
  replyToId,
  accountId,
  silent,
  mediaUrl,
...
export const discordPlugin = {
  ...
  outbound: {
    ...
    sendText: async ({ to, text, accountId, deps, replyToId, silent }) => {
      ...
      const rewrittenText = replaceEntityPaths(text);
      const result = await send(
        to,
        rewrittenText,
        buildDiscordSendArgs({ replyToId, accountId, silent }),
      );
    ...
    sendMedia: async ({ to, text, mediaUrl, accountId, deps, replyToId, silent }) => {
      ...
      const result = await send(
        to,
        rewrittenText,
        buildDiscordSendArgs({ replyToId, accountId, silent, mediaUrl }),
      );
      return { channel: "discord", ...result };
    },
  ...
  status: {
    ...
    buildAccountSnapshot: ({ account, runtime, probe, audit }) => {
      ...
      const application = runtime?.application ?? probe?.application;
