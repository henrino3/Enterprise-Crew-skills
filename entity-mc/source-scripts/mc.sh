...
normalize_output_links() {
    ...
    # Rewrite common workspace-relative paths to Entity doc links
...
# Entity-accessible directories (relative to ~/clawd)
...
case "$1" in
    ...
    review)
        ...
        # Look for patterns like output/*, memory/*, ~/clawd/*, /home/*/clawd/*
        WORKSPACE="${HOME}/clawd"
        FILE_PATHS=$(echo "$OUTPUT" | grep -oE '(output|memory)/[^ )]+\.(md|txt|html|json|pdf|png|jpg)|~/clawd/[^ )]+\.(md|txt|html|json|pdf|png|jpg)|/home/[^/]+/clawd[^/]*/[^ )]+\.(md|txt|html|json|pdf|png|jpg)')
    ...
    *)
        ...
        echo "  Baseline: ~/clawd/output/agents/estimation-engine-baseline.md"
        echo "  Accuracy: ~/clawd/output/agents/estimation-accuracy-log.md"
        echo "  Standalone: ~/clawd/scripts/estimate.sh \"task description\""
        echo ""
